...
not supply a call ID, each transcript stays separate; NoteIQ does not infer a
session from the title, calendar day, or a time gap.

## Existing data

Existing transcripts and summaries remain stored. Older combined AI summaries
are not assigned to individual sessions because they may contain multiple days.
